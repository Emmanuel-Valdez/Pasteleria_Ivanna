...
- **Content collection for cakes** (`src/content/config.ts`, entries in `src/content/cakes/*.md`): each torta is one Markdown file with frontmatter `{ title, category: 'tortas', image, imageHover?, alt }`, image(s) referenced relative to `src/assets/cakes/`. Adding a new torta to the catalog means adding one `.md` file — no component changes needed. The optional `imageHover` gives that card a second detail photo that crossfades in on hover (`CakeCard.astro`) — only a few cakes have one (e.g. `chocotorta.md`, `torta-martina-xv.md`), it's not required. Alfajores and Tartas are NOT in the collection; they're static photo+text sections in `Catalog.astro` with a WhatsApp CTA instead of a photo grid. `src/assets/tartas.png` is a real photo (tarta de frutilla) as of the July 2026 catalog refresh; `src/assets/alfajores.png` is still a low-res crop from the original Instagram flyer (`img/`) — a placeholder until a real photo exists.
- **`Catalog.astro`** assembles three pieces: a top "bento" grid (`BentoTile`) — Tortas is a large tile with a two-photo CSS crossfade on hover, plus two photo-only mini tiles linking to `#tortas` and photo tiles for Alfajores/Tartas — followed by the full Tortas photo grid (`CakeCard` per entry) and the Alfajores/Tartas text sections. `CakeCard` is a button (not a link): clicking opens a shared native `<dialog>` lightbox defined at the bottom of `Catalog.astro`, showing a larger image (`getImage` 1080w, passed via `data-full`) with a per-cake WhatsApp CTA inside the modal. The Tartas section also has a full-bleed blurred photo (`src/assets/lemonpie.png`) as an ambient backdrop behind its panel — a deliberate design accent, not a stray asset.
- **`Hero.astro`**'s `heroImg` is a manually-picked "best photo" (currently `src/assets/cakes/torta_crunchy.png`) — there's no logic choosing it, just swap the import when a better hero shot comes along.
- **New/updated cake photos**: drop raw originals in `imagenes/mejorada/` (gitignored, never committed — it's a staging area, not part of the site). Process them with the `sharp` package already in `node_modules` (pulled in transitively by `astro:assets`; this machine has no ImageMagick or Python) via a one-off `node -e` script — crop out watermarks, cap resolution around 1400px on the long side, write straight into `src/assets/cakes/`. Check recent git log for an example script if doing this again.
